 ...
 */
export const format_version = "1.7.1";
//---------------------------------------------------------------------------
// Arguments
//---------------------------------------------------------------------------
/**
...
const useBackupFolderFromAppData = flagsArgs.some((arg) => arg.toLowerCase() === "--use-app-data-backup-folder");
const fileByFileMode = flagsArgs.some((arg) => arg.toLowerCase() === "--file-by-file-mode");
/**
 ...
 */
export async function copyFolderIObitMode(folder, destination) {
    try {
        mkdirSync(destination, { recursive: true });
    }
    catch (e) { }
...
export async function getZip(versionFolder, accessMode) {
     ...
     */
    const tempPath = path.join(userFolderPath, "AppData", "Roaming", "8Crafter's Ore UI Customizer", path.basename(versionFolder), "data", "gui_vanilla_backup");
...
export async function uninstallOreUICustomizer(versionFolder) {
    if (existsSync(path.join(versionFolder, "data/gui_vanilla_backup"))) {
        if (accessType === "BedrockLauncher") {
            ...
            await runCommmand(`C:/"Program Files (x86)/IObit/IObit Unlocker/IObitUnlocker.exe" /Rename "${path.join(versionFolder, "data/gui_vanilla_backup")}" "gui"`);
        }
    }
    else {
        if (existsSync(path.join(versionFolder, "data/gui/dist/hbui/oreUICustomizer8CrafterConfig.js"))) {
