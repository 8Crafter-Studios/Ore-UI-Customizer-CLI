...
async function getOreUICustomizerAPIDataURI() {
    ...
    const dependenciesData = (await (await fetch(path.join(baseURL, "ore-ui-customizer-api.dependencies.json"))).json());
    ...
    for (const dependency of dependenciesData.dependencies) {
        const dependencyScriptData = await (await fetch(path.join(baseURL, dependency.js))).text();
